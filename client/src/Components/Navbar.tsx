...
const ProfileDropdown: React.FC<{ isDropdownOpen: boolean; closeDropdown: () => void }> = ({ isDropdownOpen, closeDropdown }) => {
  ...
  return isDropdownOpen ? (
    <div className="absolute top-full left-1/2 transform -translate-x-1/2 mt-2 w-40 bg-white border border-gray-200 rounded shadow-md z-50">
      ...
      <button className="block px-4 py-2 text-[#38436C] hover:bg-gray-100 w-full text-center" onClick={() => { handleLogout(); closeDropdown(); }}>
        Logout
      </button>
    </div>
  ) : null;
      ...
      <div className="w-2/12 hidden md:flex justify-end">
        {isAuthenticated ? (
          <div className="relative w-full flex justify-center">
            ...
            <ProfileDropdown isDropdownOpen={isDropdownOpen} closeDropdown={closeDropdown} />
          </div>
        ) : (
